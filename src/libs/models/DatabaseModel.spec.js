...
describe('DatabaseModel', () => {
  ...
  const users = Object.assign(DatabaseModel(postgres, 'users'), {
    accessibleColumns: ['name', 'username'],
  })
  ...
  describe('#_getSanitizedValue', function () {
    it(`should sanitize values appropriately to prevent XSS, plus handle JS objects turning into JSON`, function () {
      const [shouldBe1, sani1] = [
        `This is awesome`,
        users._getSanitizedValue(`This is awesome`),
      ]
      const [shouldBe2, sani2] = [
        `This is awesome `,
        users._getSanitizedValue(`This is awesome <script>alert('1')</script>`),
      ]
      const [shouldBe3, sani3] = [
        `{"lance":"yoyo"}`,
        users._getSanitizedValue({ lance: 'yoyo' }),
      ]
      const [shouldBe4, sani4] = [
        `{"lance":"yoyo "}`,
        users._getSanitizedValue({ lance: "yoyo <script>alert('2')</script>" }),
      ]
      const [shouldBe5, sani5] = [
        `{"lance":"yoyo <script>alert('2')</script>"}`,
        users._getSanitizedValue(
          JSON.stringify({ lance: "yoyo <script>alert('2')</script>" }),
          false
        ),
      ]
    ...
    it(`should replace escaped ampersands with a normal ampersand`, function () {
      assert.equal(
        `me &amp; you`,
        users._getSanitizedValue(`me & you<script>alert('1')</script>`)
      )
    })
  })
  ...
  describe('#setRecord', function () {
    ...
    it(`should set the internal record of the object to be saved or used later`, function () {
      ...
      assert.equal(undefined, users.record.username)
  ...
  describe('#getAll', function () {
    before(
      'clear users table',
      async () => await postgres.query(`truncate users cascade`)
    )
    after(
      'clear users table',
      async () => await postgres.query(`truncate users cascade`)
    ...
    it(`should get all user records in the DB`, async function () {
      ...
      await postgres.query(
        `insert into users (username) values ('lance@acme.com')`
      )

      const allUserRecords2 = await users.getAll()
      assert.equal(1, allUserRecords2.length)
      assert.equal('lance@acme.com', allUserRecords2[0].username)
  ...
  describe('#save', function () {
    ...
    it(`should create a new user without errors`, async function () {
      users.setRecord({ username: 'test1@acme.com' }, true)
    ...
    it(`should create a new user without errors and execute beforeSave callback before saving record`, async function () {
      ...
      users.setRecord({ username: 'test2@acme.com' }, true)
      const newId = await users.save()
