...
;(async function () {
  try {
    ...
    const migrationTargetDir = path.join(
      config.app.rootDir,
      '..',
      'src',
      'database',
    ...
    const modelTargetDir = path.join(
      config.app.rootDir,
      '..',
      'src',
      'libs',
    ...
    const [migrationFileContents, modelFileContents] = await Promise.all([
      readFile(path.join(__dirname, 'migrationContents.txt'), {
        encoding: 'utf-8',
      }),
      readFile(path.join(__dirname, 'modelContents.txt'), {
        encoding: 'utf-8',
      }),
    ])

    const finalMigrationContents = replaceTablePlaceholders(
      migrationFileContents
    )
    const finalModelContents = replaceTablePlaceholders(modelFileContents)

    const newMigFilename = `${moment
      .utc()
      .format('YYYYMMDDhhmmss')}_${tableName}.js`
    const finalMigrationPath = path.join(migrationTargetDir, newMigFilename)
    const finalModelPath = path.join(modelTargetDir, `${titleCaseTable}.ts`)
