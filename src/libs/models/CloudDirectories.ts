...
export default function CloudDirectories(postgres: object) {
  ...
  return Object.assign(
    factoryToExtend,
    {
      ...
      async getDirectChildren(bucketId: (number | string), directoryId: (null | number | string)) {
      ...
      async createDirsAndObjectFromFullPath(bucketId: (number | string), fullObjectPath: string): Promise<object[]> {
        const splitInfo: string[] = fullObjectPath.split('/').filter((str: string) => str !== '')
        let info: object[] = []
        for (let ind: number = 0; ind < splitInfo.length; ind++) {
          const dirOrObj: string = splitInfo[ind]
          const parentDirId: number | string = ((info[info.length - 1] as any) || {}).id

          // object name
          if (ind === splitInfo.length - 1) {
            const obj = CloudObjects(postgres)
            await obj.findOrCreateBy({ bucket_id: bucketId, full_path: fullObjectPath })
          ...
          await dir.findOrCreateBy({ bucket_id: bucketId, full_path: splitInfo.slice(0, ind + 1).join('/') })
